# Update SSH Config for server
#
# Usage:
#
#    ripcord update-ssh-config [IP4] [NAME] [PASSWORD]
#
# - [IP4]      : Server IP4 address
# - [NAME]     : Server name. Example: "green"
# - [PASSWORD] : Root user password
#
# This script:
# - Updates the local SSH config file so we can SSH into the server using it's
#   name. For example `ssh root@NAME`
# - Removes the provided IP4 from the ssh known hosts file. This is assuming
#   the server will have a new identity.
# - Install sshpass on our local machine if we don't have it installed already
# - Copy our ssh key to server so we can connect without a password
# - Disable SSH password authentication on the server


## Read script Args

echo "Update SSH Config for Server"
echo ""

read --prompt-str "Server IP4 address: " --list server_ip4
read --prompt-str "Server name: " --list server_name
read --prompt-str "Password for root@$server_name: " --silent --list server_password



## Pre-define global variables ##

set server_ip4 $argv[1]
set server_name $argv[2]
set server_password $argv[3]
...
function disable_password_authentication
  ...
  set ssh_command "grep -q '$find_line' '$file_path' && sed 's|$find_line|$replace_line|' '$remote_file_path' > '$temp_file' && mv '$temp_file' '$remote_file_path' || echo 'Error: Line not found.' >&2"
